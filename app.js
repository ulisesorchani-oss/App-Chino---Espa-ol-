...
function expandWordCards(key, rows) {
    ...
    _wordCache[key] = rows.map((r, i) => {
        ...
        return {
            ...
            level: Number(lv),
            module: key,
            w: 1, // tarjeta de palabra (HSK 3.0): el motor la muestra como traducción directa
...
async function loadSentences() {
    ...
    if (typeof EMBEDDED_MODULE_DATA !== 'undefined' && EMBEDDED_MODULE_DATA[state.activeModule]) {
        ...
        data = included;
        const unit = state.activeModule.startsWith('HSK') ? 'palabras' : 'oraciones';
...
function renderSentenceText(s) {
    ...
    if (learningChinese) {
        // v7.19: tarjeta de palabra (HSK 3.0) → el prompt es la palabra ESPAÑOLA y el
        // alumno escribe el hanzi (el pinyin de la palabra sirve de guía si está ON).
        if (s.w) {
    ...
    // v7.19: tarjeta de palabra → se muestra el hanzi (con colores de tono si están
    // activos) y el alumno escribe la palabra en español.
    if (s.w) {
        const zhWord = s['chinese_' + k + '_full'] || '';
        if (showToneColors && typeof pinyinPro !== 'undefined') {
            try {
                ...
                sentenceTextEl.innerHTML = html;
            } catch (e) { sentenceTextEl.textContent = zhWord; }
        } else {
            sentenceTextEl.textContent = zhWord || 'Error en datos';
        }
        return;
    }
...
function showFullTranslation() {
    ...
    // v7.19: en tarjetas de palabra el “prompt” ya es una lengua; la revelación
    // muestra la OTRA lengua (el hanzi buscado o la palabra española).
    if (s.w) translationText = learningChinese ? s['chinese_' + k + '_full'] : s.spanish_full;
